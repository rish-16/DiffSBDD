...
def compute_hessian(coord, force_field, device, use_cell_list=False):
    ...
    hessian[pairs[:,0], pairs[:,1]] = (
        ...
        / sq_dist.view(-1, 1, 1)
        * disp.view(-1,3,1) * disp.view(-1, 1, 3)
    )    
    # Set values for main diagonal
    indices = torch.arange(len(coord), device=device)
    hessian[indices, indices] = -torch.sum(hessian, dim=0) # was axis
    ...
    # hessian = torch.transpose(hessian, (0, 2, 1, 3)) \
...
def _prepare_values_for_interaction_matrix(coord, force_field, device, use_cell_list):
    ...
    if cutoff_distance is None:
        ...
        # dupa = struc.displacement(
        #     coord.numpy()[np.newaxis, :, :], coord.numpy()[:, np.newaxis, :]
        # )
        
        disp_matrix = torch.cdist(
            coord.view(1, -1, 3), coord.view(-1, 1, 3), p=2
        )
        # sq_dist_matrix = torch.sum(disp_matrix * disp_matrix, dim=-1) # was axis
    ...
    # pairs = torch.tensor((atom_i, atom_j)).T
    # pairs = torch.tensor(atom_i, atom_j).T
